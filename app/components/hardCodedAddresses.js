...
//Street Address,Elementary,Status,Listing ID,List Price,Sale Price,Above ask,List Date,Close of Escrow Date,Days On Market,Bedrooms,Bathrooms,Sq Ft,price sq ft,Lot Size Edit,Area,City
    var rawData = [
        {
    ...
    function addCoordinate (item) {
        var short = item.shortAddress.replace(/\s+/g, '+');
        var url   = 'https://maps.googleapis.com/maps/api/geocode/json'
        + '?address='+short+',+Sunnyvale,+CA'
        + '&key='+Config.Google.GeocodeAPI;
        return $http.get(url)
        .then(
            function (data) {
                var info = data.data.results[0];
                item.address  = info.formatted_address;
                item.location = info.geometry.location;
                item.bbox     = {
                    ur : info.geometry.viewport.northeast,
                    ll : info.geometry.viewport.southwest
                };
                return item;
        });
    }
...
//         if (i == 3) break;
        starts[i] = $q.defer();
        promises[i] = starts[i].promise.then( addCoordinate );
    };
    ...
    var inflight = 0;
    function startFunc () {
        if ( next >= promises.length ) return;

        if ( inflight < limit ) {
            starts[next].resolve(rawData[next]);
            inflight++;
            promises[next].finally( function () { console.log('.'); inflight-- } );
            next++;
        }
